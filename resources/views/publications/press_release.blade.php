...
@push('styles')
    ...
    <style>
        ...
        .header img {
            ...
            left: 0;
            width: 100%;
            /* Ensure the image spans the entire width */
            height: 100%;
            /* Ensure the image spans the entire height */
            object-fit: cover;
            /* Make sure the image covers the container without stretching */
            z-index: -1;
            /* Push the image behind the text */
        }
        ...
        .blog-post-page img {
            ...
            height: 250px;
            cursor: pointer;
            /* Add a pointer cursor to indicate the image is clickable */
                        ...
                        @foreach($pressReleases as $pressRelease)
                            ...
                            <div class="blog-post-page">
                                @if (!empty($pressRelease->image) && file_exists(public_path('storage/' . $pressRelease->image)))
                                    <!-- Clickable image linking to full-size view -->
                                    <a href="{{ asset('storage/' . $pressRelease->image) }}" target="_blank">
                                        <img src="{{ asset('storage/' . $pressRelease->image) }}" alt="Blog image">
                                    </a>
                                @elseif (!empty($pressRelease->image) && file_exists(public_path('uploads/' . $pressRelease->image)))
                                    <a href="{{ asset('uploads/' . $pressRelease->image) }}" target="_blank">
                                        <img src="{{ asset('uploads/' . $pressRelease->image) }}" alt="Blog image">
                                    </a>
                                @else
                                    <img src="{{ asset('assests/event.jpeg') }}" alt="Default image">
                                @endif
                                ...
                                @if (!empty($pressRelease->image) && file_exists(public_path('storage/' . $pressRelease->image)))
                                    <a href="{{ asset('storage/' . $pressRelease->image) }}" download class="download-button">
                                        <i class="fa-solid fa-download"></i> Download Image
                                    </a>
                                @elseif (!empty($pressRelease->image) && file_exists(public_path('uploads/' . $pressRelease->image)))
                                    <a href="{{ asset('uploads/' . $pressRelease->image) }}" download class="download-button">
                                        <i class="fa-solid fa-download"></i> Download Image
                                    </a>
                                @else
                                    <span>No Image available</span>

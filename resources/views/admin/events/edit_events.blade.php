                                ...
                                <form action="{{ route('events.update', $event->id) }}" method="POST"
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control @error('order') is-invalid @enderror"
                                            id="order" name="order" value="{{ old('order', $event->order) }}" required>
                                        @error('order')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        @if (!empty($event->image))
                                                                            @php
                                                                                $imagePath = file_exists(public_path('uploads/' . $event->image))
                                                                                    ? asset('uploads/' . $event->image)
                                                                                    : (file_exists(public_path('storage/' . $event->image))
                                                                                        ? asset('storage/' . $event->image)
                                                                                        : asset('assets/event.jpeg'));
                                                                            @endphp
                                                                            <img src="{{ $imagePath }}" class="mt-2" width="150">
                                        @endif
                                    </div>
